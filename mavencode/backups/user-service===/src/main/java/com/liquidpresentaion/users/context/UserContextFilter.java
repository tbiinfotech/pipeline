package com.liquidpresentaion.users.context;
...
public class UserContextFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
		...
		if (principal instanceof Map) {
			...
			List idList = (List)user.get("supplierGroupIds");
			if (!idList.isEmpty()) {
				for (Object object : idList) {
					if (object instanceof  Integer) {
						intIdList.add((Integer)object);
					} else {
						intIdList.add(Integer.valueOf(object + ""));
					}
				}
			}
			UserContextHolder.getContext().setSupplierGroupIds(intIdList);
		}
